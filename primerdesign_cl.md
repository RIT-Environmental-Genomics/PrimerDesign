...
To access the cluster remotely, you will use <span style="color:lightblue">ssh</span>. Remember to change "XXX" to your assigned username.
```
...
> **NOTE:** This is an extremely simplified process. Primer3_core is designed to pick primers from one sequence at a time. If you wanted to design universal primers (e.g., a single primer pair common to multiple sequences) then you would need to include additional steps before primer3 (e.g., aligning sequences, generating a consensus sequence) and after primer3 (e.g., automating consolidation of primer candidate, checking specificity against a database, like BLAST). Luckily, there are some great GUI tools, like Primer-BLAST on NCBI, that already do this for you no need to reinvent the wheel! Hopefully, you now understand the behind-the-scenes process of primer design whether in the command line or a GUI.